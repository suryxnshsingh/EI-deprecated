...
const SubCard = ({ id, code, name, session, semester, onClick }) => {
  const navigate = useNavigate();
    return (
      <div
        onClick={() => {
          if (onClick) {
            onClick(id, code, name);
          } else {
            navigate(`${id}`);
          }
        }}
      ...
      >
        ...
        {(session || semester) && (
          <p className="text-xs text-slate-400 group-hover:text-violet-200 relative z-10 duration-300 mt-1">
